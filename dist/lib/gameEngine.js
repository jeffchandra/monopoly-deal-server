...
exports.dealOpeningHands = dealOpeningHands;
exports.startGame = startGame;
exports.startTurn = startTurn;
exports.checkWinCondition = checkWinCondition;
exports.endTurn = endTurn;
exports.discard = discard;
exports.placePropertyAsNewSet = placePropertyAsNewSet;
exports.placePropertyIntoSet = placePropertyIntoSet;
exports.playCardToBank = playCardToBank;
exports.confirmPayment = confirmPayment;
exports.getPayableSources = getPayableSources;
exports.playRentCard = playRentCard;
exports.placePendingProperty = placePendingProperty;
exports.playPassGo = playPassGo;
exports.playItsMyBirthday = playItsMyBirthday;
exports.playDebtCollector = playDebtCollector;
exports.playSlyDeal = playSlyDeal;
exports.playForcedDeal = playForcedDeal;
exports.playHouse = playHouse;
exports.playHotel = playHotel;
exports.movePropertyBetweenSets = movePropertyBetweenSets;
exports.playWildRent = playWildRent;
exports.moveWildToNewColor = moveWildToNewColor;
...
function getOpponents(game, playerId) {
...
function drawCard(game, playerId) {
    if (game.deck.length === 0) {
        if (game.discardPile.length === 0)
            return;
        game.deck = (0, deck_1.shuffleDeck)([...game.discardPile]);
        game.discardPile = [];
...
function startGame(game) {
    game.deck = (0, deck_1.shuffleDeck)((0, deck_1.createDeck)());
...
function checkWinCondition(game) {
    ...
    const completedSets = (0, propertyUtils_1.getCompletedSetCount)(current);
...
function placePropertyAsNewSet(game, playerId, cardId, colorOverride) {
...
function placePropertyIntoSet(game, playerId, cardId, setId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
    ...
    if ((0, propertyUtils_1.isSetComplete)(set))
        throw new Error("That set is already complete");
    const card = player.hand.splice(cardIdx, 1)[0];
    ...
    if (card.colors.length > 1) {
        ...
        if (card.activeColor !== set.color)
            throw new Error("Card color does not match set");
    }
...
function playCardToBank(game, playerId, cardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
...
function getPayableSources(game, playerId) {
    const player = getPlayerById(game, playerId);
    const bankCards = [...player.bank];
    const incompleteSetCards = [];
...
function playRentCard(game, playerId, cardId, setId, doubleRentCardId, targetPlayerId // if provided, only charge this player (Wild Rent)
) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
    ...
    if (card.type !== "rent")
        throw new Error("Card is not a rent card");
    const rentCard = card;
    const set = player.propertySets.find(s => s.id === setId);
    if (!set)
        throw new Error("Set not found");
    ...
    const rentableSets = (0, propertyUtils_1.getRentableSetsByCard)(player, rentCard);
    ...
    if (doubleRentCardId) {
        ...
        if (doubleIdx === -1)
            throw new Error("Double Rent card not found in hand");
        const doubleCard = player.hand.splice(doubleIdx, 1)[0];
        game.discardPile.push(doubleCard);
        multiplier = 2;
    }
    const amount = (0, propertyUtils_1.getRentForSet)(set) * multiplier;
    ...
    addLog(game, `${player.name} charged ${targetPlayerId
        ? getPlayerById(game, targetPlayerId).name
        : "everyone"} $${amount}M rent on ${propertyRules_1.PROPERTY_RULES[set.color].displayName}${multiplier === 2 ? " (doubled!)" : ""}.`);
}
function placePendingProperty(game, playerId, cardId, targetSetId) {
    const player = getPlayerById(game, playerId);
    const cardIdx = player.pendingPlacements.findIndex(c => c.id === cardId);
    if (cardIdx === -1)
        throw new Error("Card not in pending placements");
...
function playPassGo(game, playerId, cardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
...
function playItsMyBirthday(game, playerId, cardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
...
function playDebtCollector(game, playerId, cardId, targetPlayerId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
...
function playSlyDeal(game, playerId, cardId, targetPlayerId, targetSetId, targetCardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
...
function playForcedDeal(game, playerId, cardId, targetPlayerId, targetSetId, targetCardId, offeredSetId, offeredCardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
    ...
    if (!offeredSet)
        throw new Error("Offered set not found");
    if ((0, propertyUtils_1.isSetComplete)(offeredSet))
        throw new Error("Cannot give away a card from a complete set");
    const offeredCardIdx = offeredSet.properties.findIndex(c => c.id === offeredCardId);
    if (offeredCardIdx === -1)
        throw new Error("Offered card not found in set");
    // Remove card from hand and discard
    const card = player.hand.splice(cardIdx, 1)[0];
    game.discardPile.push(card);
    game.actionsRemaining--;
...
function playHouse(game, playerId, cardId, setId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
    ...
    if (!set)
        throw new Error("Set not found");
    if (!(0, propertyUtils_1.isSetComplete)(set))
        throw new Error("Can only add a house to a complete set");
    ...
    set.hasHouse = true;
    game.actionsRemaining--;
    addLog(game, `${player.name} added a house to their ${propertyRules_1.PROPERTY_RULES[set.color].displayName} set. Rent is now $${(0, propertyUtils_1.getRentForSet)(set)}M.`);
}
function playHotel(game, playerId, cardId, setId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
    ...
    if (!set)
        throw new Error("Set not found");
    if (!(0, propertyUtils_1.isSetComplete)(set))
        throw new Error("Can only add a hotel to a complete set");
    if (set.color === "railroad" || set.color === "utility") {
        throw new Error("Cannot add hotels to railroads or utilities");
    }
    if (!set.hasHouse)
        throw new Error("Must have a house before adding a hotel");
    if (set.hasHotel)
    ...
    set.hasHotel = true;
    game.actionsRemaining--;
    addLog(game, `${player.name} added a hotel to their ${propertyRules_1.PROPERTY_RULES[set.color].displayName} set. Rent is now $${(0, propertyUtils_1.getRentForSet)(set)}M.`);
}
function movePropertyBetweenSets(game, playerId, cardId, fromSetId, toSetId) {
    const player = getPlayerById(game, playerId);
    const fromSet = player.propertySets.find(s => s.id === fromSetId);
    if (!fromSet)
        throw new Error("Source set not found");
    if ((0, propertyUtils_1.isSetComplete)(fromSet))
        throw new Error("Cannot move a card from a complete set");
    const toSet = player.propertySets.find(s => s.id === toSetId);
    if (!toSet)
        throw new Error("Destination set not found");
    if ((0, propertyUtils_1.isSetComplete)(toSet))
        throw new Error("Cannot move a card into a complete set");
    ...
    if (cardIdx === -1)
        throw new Error("Card not found in source set");
    ...
    if (card.colors.length > 1) {
        ...
        if (fromSet.color !== toSet.color)
            throw new Error("Sets must be the same color");
    }
    if (fromSet.properties.length === 0) {
        player.propertySets = player.propertySets.filter(s => s.id !== fromSetId);
    }
    toSet.properties.push(card);
    addLog(game, `${player.name} moved ${card.name} between ${propertyRules_1.PROPERTY_RULES[toSet.color].displayName} sets.`);
    checkWinCondition(game);
}
function playWildRent(game, playerId, cardId, setId, targetPlayerId, doubleRentCardId) {
    if (game.phase !== "actionPhase")
        throw new Error("Not in action phase");
    if (game.actionsRemaining <= 0)
        throw new Error("No actions remaining");
    ...
    const card = player.hand[cardIdx];
    if (card.type !== "action")
        throw new Error("Card is not a Wild Rent card");
    const set = player.propertySets.find(s => s.id === setId);
    if (!set)
        throw new Error("Set not found");
    const target = getPlayerById(game, targetPlayerId);
    ...
    game.phase = "pendingAction";
    addLog(game, `${player.name} charged ${target.name} $${amount}M Wild Rent on ${propertyRules_1.PROPERTY_RULES[set.color].displayName}${multiplier === 2 ? " (doubled!)" : ""}.`);
}
function moveWildToNewColor(game, playerId, cardId, fromSetId, newColor) {
    const player = getPlayerById(game, playerId);
    const fromSet = player.propertySets.find(s => s.id === fromSetId);
    if (!fromSet)
        throw new Error("Source set not found");
    if ((0, propertyUtils_1.isSetComplete)(fromSet))
        throw new Error("Cannot move from a complete set");
    const cardIdx = fromSet.properties.findIndex(c => c.id === cardId);
    if (cardIdx === -1)
    ...
    addLog(game, `${player.name} moved ${card.name} to a new ${propertyRules_1.PROPERTY_RULES[newColor].displayName} set.`);
    checkWinCondition(game);
}
